...
class Spider():
    # 站点的url
    url = 'https://www.huya.com/g/2633'

    # 数据的获取，获取房间名room、主播anchor、观看人数number，存入room_info_list二维数组中
    def __fetch_content(self):
        r = request.urlopen(Spider.url)
        html = str(r.read(), encoding='utf-8')
        root_content = re.findall('<li class="game-live-item" gid="2633">([\s\S]*?)</li>',html)
    ...
    def __rank(self, room_info_list):
        ...
        def pre_rank(room_info):
            number = room_info[2]
            if '万' in str(room_info[2]):
                number = re.sub('万', '', room_info[2], 0)
                number = float(number) * 10000
            return float(number)
        room_info_list = sorted(room_info_list, key=lambda room_info: pre_rank(room_info), reverse=True)
        # 遍历打印排序后的结果
        for i in range(0, len(room_info_list)):
            print(room_info_list[i])

    # 主方法，从这个方法开始
    def go(self):
        room_info_list = self.__fetch_content()
        self.__rank(room_info_list)
