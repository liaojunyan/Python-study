...
class Spider:
    ...
    def __fetch_content():
        ...
        number = re.findall('<span class="num"><i class="num-icon"></i><i class="js-num">'
                            '([\s\S]*?)</i></span>',root_content_str)
        room_info_list = []
    ...
    def __show(self, room_info_list):
        ...
        for i in range(0, len(room_info_list)):
            print('rank' + str(i + 1) + ': ' + room_info_list[i][0] + ' ' + room_info_list[i][1] + ' ' +
                  room_info_list[i][2])
